...
// go 2 hell forest
export default function zoomReducer( actions, props, oldState, currentState, next ) {

    // Is the zoomIn reducer already running?
    if( oldState.cameraPositionZoomIn ) {

        return next( zoomIn( actions, props, oldState, currentState ) );

    // Is the zoom out reducer already running?
    } else if( oldState.cameraPositionZoomOut ) {

        return next( zoomOut( actions, props, oldState, currentState ) );

    }

    const zoomInState = zoomIn( actions, props, oldState, currentState );
